...
class Data_cleaning:
  ...
  def __init__ (self, MT_data, DAT_data, zip_data):
    ...
    self.required_column = ['CUSTOMER', 'DISTANCE', 'CASES', 'WEIGHT', 'VOLUME', 
        'ACTUAL CARRIER', 'ACTUAL MODE','ACTUAL EQUIP', 'PU_APPT',
        'DL_APPT', 'SPOT_AVG_LINEHAUL_RATE', 'SPOT_LOW_LINEHAUL_RATE',
       ...
       'CONTRACT_YOUR_OWN_AVG_LINEHAUL_RATE', 'ORIGIN_LAT', 'ORIGIN_LNG', 'DEST_LAT', 'DEST_LNG', 'LINEHAUL COSTS']
  ...
  def clean_mt_data(self):
    ...
    self.mt = self.mt[self.mt['ORIGIN STATE'].isin(self.us_list + self.canada_list) &\
                      self.mt['DEST STATE'].isin(self.us_list + self.canada_list)]
    ...
    self.mt = self.mt.drop(['Unnamed: 0.1','Unnamed: 0'], axis = 1)
    ...
    self.mt = self.mt[pd.notnull(self.mt['ORIGIN CITY'])]

    self.mt = self.mt[pd.notnull(self.mt['LINEHAUL COSTS'])]
    self.mt = self.mt[pd.notnull(self.mt['ORIGIN STATE'])]
    ...
    self.mt = self.mt[pd.notnull(self.mt['FUEL COSTS'])]

    # Remove any nan cell in ACC. COSTS
    self.mt = self.mt[pd.notnull(self.mt['ACC. COSTS'])]

    # Remove any nan cell in total ACTUAL COST
    self.mt = self.mt[pd.notnull(self.mt['TOTAL ACTUAL COST'])]

    # Add a LINEHAUL UNIT COSTS to the dataframe
    self.mt['LINEHAUL UNIT COSTS'] = self.mt['LINEHAUL COSTS']/self.mt['DISTANCE']

    # Drop the row if na is found in the MT_data['DEST CITY']
    self.mt = self.mt[pd.notnull(self.mt['DEST CITY'])]

    # Only conisder shipment has dest zip code in USA and Canada
    # Drop the row if ns is found in the MT_data['ORIGIN STATE']
    self.mt = self.mt[pd.notnull(self.mt['DEST STATE'])]
    self.mt = self.mt.reset_index(drop=True)
    ...
    self.mt = pd.merge(self.mt, self.zip_data, how = 'left', left_on = ['ORIGIN ZIP', 'ORIGIN STATE'], right_on = ['zipcode', 'state'])
    self.mt = pd.merge(self.mt, self.zip_data, how = 'left', left_on = ['DEST ZIP', 'DEST STATE'], right_on = ['zipcode', 'state'])
  ...
  def clean_dat_data(self):
    # first, drop the data that has wrong origin state or destination state
    self.dat = self.dat[self.dat['ORIG_STATE'].isin(self.us_list + self.canada_list) &\
                      self.dat['DEST_STATE'].isin(self.us_list + self.canada_list)]
  ...
  def final_data(self):
    ...
    merge_df_remove_NA = merge_df_remove_NA[self.required_column]
